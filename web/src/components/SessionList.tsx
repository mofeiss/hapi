...
function SessionItem(props: {
  ...
  const { archiveSession, renameSession, deleteSession, isPending } =
    useSessionActions(api, s.id, s.metadata?.flavor ?? null);
      ...
      >
        ...
        >
          ...
          <span className="inline-flex items-center gap-1 truncate">
            ...
            <span className="truncate" title={s.metadata?.path}>
              {s.metadata?.path
                ? getPathDisplayName(s.metadata.path)
                : s.id.slice(0, 8)}
            </span>
          </span>
        </div>
      ...
      {!batchMode ? (
        <>
          <SessionActionMenu
            ...
            onDelete={() =>
              skipDeleteConfirm ? handleQuickDelete() : setDeleteOpen(true)
            }
